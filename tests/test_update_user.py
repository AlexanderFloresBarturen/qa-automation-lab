...
def test_update_user_not_found(client, loged_user, valid_update_payload):
    headers = {}
    headers["Authorization"] = f"Bearer {loged_user["token"]}"
    response = client.put("/profile/99999", headers=headers, json=valid_update_payload)
    body = response.json()

    assert response.status_code == 404

    assert_not_found_response(body)


def test_update_user_duplicate_email(client, user_payload, loged_user, valid_update_payload):
    user_two = user_payload()

    post_response = client.post("/auth/register", json=user_two)
    body_post = post_response.json()

    headers = {}
    headers["Authorization"] = f"Bearer {loged_user["token"]}"
    update_payload = valid_update_payload.copy()
    update_payload["email"] = body_post["email"]
    put_response = client.put("/profile", headers=headers, json=update_payload)
...
def test_update_user_without_token(client, created_user, valid_update_payload):
    update_response = client.put("/profile", json=valid_update_payload)
...
def test_update_user_invalid_token(client, created_user, valid_update_payload):
    update_response = client.put("/profile", headers={"Authorization": "Bearer invalid_token"}, json=valid_update_payload)
    update_body = update_response.json()
...
def test_update_another_user(client, user_payload, loged_user, valid_update_payload):
    ...
    headers = {}
    headers["Authorization"] = f"Bearer {loged_user["token"]}"
    update_response = client.put(f"/profile/{created_body_two['id']}", headers=headers, json=valid_update_payload)
    update_body = update_response.json()

    assert created_response_two.status_code == 201
    assert update_response.status_code == 404

    assert_not_found_response(update_body)
